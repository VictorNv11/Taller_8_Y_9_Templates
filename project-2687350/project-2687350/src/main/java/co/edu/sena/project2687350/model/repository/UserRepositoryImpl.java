...
public class UserRepositoryImpl implements Repository<User>{
    ...
    public List<User> listAllObj() throws SQLException {
        sql ="select u.user_id, u.user_firstname, u.user_lastname, u.user_email, u.user_password from users_tbl u order by u.user_lastname, u.user_firstname, u.user_password ";
    ...
    if (user.getUser_id() != null && user.getUser_id()> 0 ){
        sql = "update users_tbl set user_firstname = ?, user_lastname = ?, user_email = ?," +
              "user_password = aes_encrypt (?, '$2a$12$KLaAbpUXVQqcY24yn7/V3.E/3gorGTEHBhFqQvupA7Pct6Seschrm') " +
              "where  user_id= ?";
    }else{
        sql = "insert into users_tbl (user_firstname, user_lastname, user_email, user_password) " +
               "values (upper(?), upper(?), lower(?), " +
               "aes_encrypt(?, '$2a$12$KLaAbpUXVQqcY24yn7/V3.E/3gorGTEHBhFqQvupA7Pct6Seschrm'))";
        }
    try (Connection conn = ConnectionPool.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
